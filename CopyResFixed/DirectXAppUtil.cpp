...
static HANDLE g_pSharedHandle = nullptr;

#pragma region Установки и пр.
...
HRESULT AppClass::InitDevice(UINT width, UINT height)
{
	...
	// Переходим к созданию буфера глубин. Создаем текстуру-описание буфера глубин:
...
void AppClass::UpdateLight()
{
	...
	else
	{
		...
		// тут я добавил последний множитель - (1 + 3 * (g_uAppNumber % 2))
		t = ((dwTimeCur - dwTimeStart + g_uAppNumber * 2) / 1000.0f) * (1 + 7 * (g_uAppNumber % 2));
	}
...
void AppClass::Render()
{
		...
		if (g_uAppNumber == 1)
		{
			...
			if (pMainResource)
			{
				...
				if (g_pSharedTexture)
				{
					...
					// get 'Shared Handle' from 'Shared Texture':
			...
			//// нужно чтобы операция копирования в этом потоке завершилась
			//Sleep(1);
	...
	else
	{
		if (g_pSharedHandle)
		{
			...
			// надо подождать чтобы копирование в пред. потоке завершилось (мьютексы не работают)
			Sleep(1);

			// получаем временный ресурс, куда ранее скопировали соотв. текстуру:
			ID3D11Resource* pSharedResource = nullptr;
			if (FAILED(g_pd3dDevice->OpenSharedResource(g_pSharedHandle, __uuidof(ID3D11Resource),
