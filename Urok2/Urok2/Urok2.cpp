...
HRESULT InitDevice();										// Иниц-ция устройств DirectX
HRESULT InitGeometry();										// Иниц-ция шаблона ввода и буфера вершин
...
HRESULT InitGeometry()
{
	...
	if (FAILED(hr))
	{
		MessageBox(NULL, L"Невозможно скомпилировать файл FX. \
				   Пожалуйста, запустите данную программу из папки, содержащей файл FX.",
				   L"Ошибка", MB_OK);
		return hr;
	}
	...
	// текстуры. Шаблон вершин указывает, какие именно параметры содержат вершины, которые мы
	// собираемся использовать. Наши вершины (SimpleVertex) содержат только информацию о координатах
	// в пространстве. Здесь же мы указываем вершинный шейдер, который будет использоваться для
	// обработки информации о наших вершинах.
	...
	// Буфер вершин: установим его как источник вершин для рисования в устройстве рисования (Device Context)
