...
HRESULT InitDevice()
{
	...
	// для того, чтобы выяснить, какой режим обработки 3D поддерживает наш компьютер.
	...
	// Я уже упоминал, что интерфейс g_pd3dDevice будет использоваться для создания остальных объектов
